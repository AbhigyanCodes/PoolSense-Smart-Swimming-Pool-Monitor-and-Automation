...
def ensure_csv():
    ...
    if not os.path.exists(LOG_CSV):
        with open(LOG_CSV, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["timestamp","waterLevel","pH","dhtTemp","dhtHum","dsTemp","turbidity","chlorine"])
...
def check_alerts(payload):
    ...
    if payload.get("pH") is not None:
        if payload["pH"] < THRESHOLDS["pH_min"] or payload["pH"] > THRESHOLDS["pH_max"]:
            alerts.append(("pH", payload["pH"]))
    if payload.get("turbidity") is not None and payload["turbidity"] > THRESHOLDS["turbidity_max"]:
        alerts.append(("turbidity", payload["turbidity"]))
    if payload.get("chlorine") is not None and payload["chlorine"] < THRESHOLDS["chlorine_min"]:
